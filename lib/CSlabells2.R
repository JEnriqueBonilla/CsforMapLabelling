CSlabells2<-function (x, y = NULL, labels = seq(along = x), cex = 1, method = c("CS" 
...
SAmax=samax, InitialSol=inisol, ...) 
{
  ####una etiqueta a modificar
  #### se modifica el concepto de radio y se añade el concepto de lejanía
  #### se conserva la modificación del centro y no sólo del gen
  ...
  cs <- function() {
    ...
    mdist <- as.matrix(dist(cbind(x,y), method = "euclidean", diag = T, upper =T))
    
    LocalSearch <- function(geneCe){
      
      lscenter <-sample(length(x), 1)
      
      neightbours <- which(mdist[lscenter,] %in% sort(mdist[lscenter,])[1:floor(length(x)/4)])
    ...
    while (T0 > TE) {
      bestscoreant<-bestscore #condición de no cambio 
      #Simulated Annealing
      for (j in 1:SAmax) {
        newgene <- gene
        ...
        #Nota: se le añade esto al código del Paper (2014)
        if (score <= bestscore) {
          bestscore <- score
          bestgene <- gene
        }
      ...
      Pmax<-which(dist== max(dist)) #posición de el/los Centro(s) más cercano(s) a S
      ...
      if(Tau[Pmax[1]]==Taumax){
        ...
        if(score==scoreC[Pmax[1]]){
          ...
          if(B[Pmax[1]]==Bmax){
            ...
            C[[Pmax[1]]][sample(1:n_labels, 1)] <- sample(1:8, 1)
