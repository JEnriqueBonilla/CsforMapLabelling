##### las funciones estan en la carpeta funciones_valor_objetivo
...
ggplot(data.graf, aes(x= id))+geom_line(aes(y = log(mcs,10), color = "MCS"))+
  ...
  geom_point(aes(x=50, y = log(34.63116, 10)), color ="red")+
  geom_text(aes(x=50, y = log(34.63116, 10), label = "Punto de paro, Pointlabel"),hjust=-.03,just=0, size=5, fontface="plain")+
  # geom_text(aes(label=ifelse(log(pointlabel,10)==log(34.63116, 10), "sANN",'')),hjust=0,just=0) +
  xlab("Iteración") + ylab("Logaritmo base 10 de la función objetivo")
